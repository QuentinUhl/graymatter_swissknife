...
def moving_d0_sphere_jacobian(r, D0, b, big_delta, small_delta):
    """
    (c) Quentin Uhl, July 2023
    ...
    dt_ref_dd0 = -r ** 2 / D0 ** 2
    d_inv_t_ref_dd0 = 1 / r ** 2
    bardelta_dd0 = np.expand_dims(small_delta * d_inv_t_ref_dd0, axis=-1)
    bar_bigdelta_dd0 = np.expand_dims(big_delta * d_inv_t_ref_dd0, axis=-1)
    ...
    mlnS_dr = np.sum((2 / (alpha ** 6 * (alpha ** 2 - 2))) *
                     (2 * alpha ** 2 * bardelta_dr + 2 * ((-alpha ** 2 * bardelta_dr) * np.exp(-alpha ** 2 * bardelta) +
                      ...
                      (-alpha ** 2 * (bardelta_dr + bar_bigdelta_dr)) * np.exp(-alpha ** 2 * (bardelta + bar_bigdelta)) -
                      (-alpha ** 2 * (bar_bigdelta_dr - bardelta_dr)) * np.exp(-alpha ** 2 * (bar_bigdelta - bardelta))), axis=-1)
    mlnS_dd0 = np.sum((2 / (alpha ** 6 * (alpha ** 2 - 2))) *
                     (2 * alpha ** 2 * bardelta_dd0 + 2 * ((-alpha ** 2 * bardelta_dd0) * np.exp(-alpha ** 2 * bardelta) +
                                                          (-alpha ** 2 * bar_bigdelta_dd0) * np.exp(-alpha ** 2 * bar_bigdelta)) -
                      (-alpha ** 2 * (bardelta_dd0 + bar_bigdelta_dd0)) * np.exp(-alpha ** 2 * (bardelta + bar_bigdelta)) -
                      (-alpha ** 2 * (bar_bigdelta_dd0 - bardelta_dd0)) * np.exp(-alpha ** 2 * (bar_bigdelta - bardelta))), axis=-1)
    
    mlnS_dr = mlnS_dr * D0 * g ** 2 * t_ref ** 3 + 3 * mlnS * D0 * g ** 2 * t_ref ** 2 * dt_ref_dr
    mlnS_dd0 = mlnS_dd0 * D0 * g ** 2 * t_ref ** 3 + mlnS * g ** 2 * t_ref ** 3 + 3 * mlnS * D0 * g ** 2 * t_ref ** 2 * dt_ref_dd0
    
    mlnS = mlnS * D0 * g ** 2 * t_ref ** 3
    signal = np.exp(-mlnS)
    
    jacobian_dr = -mlnS_dr * signal
    jacobian_dd0 = -mlnS_dd0 * signal
    return signal, jacobian_dr, jacobian_dd0
